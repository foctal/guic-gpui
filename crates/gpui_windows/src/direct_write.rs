...
mod tests {
    ...
    fn color_emoji_rasterization_is_stable_across_batches() -> Result<()> {
        ...
        eprintln!("rasterized {} Segoe UI Emoji glyphs", params_list.len());
        for (_, bitmap) in &first {
            assert!(
                bitmap
                    .chunks_exact(4)
                    .any(|p| p[3] != 0 && (p[0] != p[1] || p[1] != p[2])),
                "expected visible color glyph pixels"
            );
            assert!(
                bitmap.chunks_exact(4).any(|p| p == [0, 0, 0, 0]),
                "expected transparent pixels outside the glyph"
            );
        }
    ...
    fn color_emoji_composites_over_cleared_texture() -> Result<()> {
        ...
        for (index, pixel) in rasterized.chunks_exact(4).enumerate() {
            let covered = index % (SIZE as usize) < 4 && index / (SIZE as usize) < 4;
            assert_eq!(
                pixel,
                if covered {
                    &[255, 255, 255, 255]
                ...
                "unexpected composited pixel at index {index}"
            );
        }
        Ok(())
    }
